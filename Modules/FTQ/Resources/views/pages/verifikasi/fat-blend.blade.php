...
@push('scripts')
    ...
    <script>
        ...
        function getShift(){
            switch (true) {
                case (moment().format('h:mm:ss') > '07:00:00' || moment().format('h:mm:ss') < '15:29:59'):
                        $('input[name="shift[]"]').val('1');
                    break;
                case (moment().format('h:mm:ss') > '15:30:00' || moment().format('h:mm:ss') < '22:29:59'):
        ...
        function create(){
            ...
            $('table#pic').find('tbody').append('<tr>'+
                    ...
                    '<td><input class="form-control" name="processmix[]" type="text" pattern="[1-99]*-[1-99]*" placeholder="1-x" data-parsley-required/></td>'+
            ...
            $.get(editUrl, function(response){
                ...
                $.each(response.data.trfatblend, function(key, val){
                    td += '<tr>'+
                            ...
                            '<td align="center"><input class="form-control" type="text" name="txtUom[]" value="'+val.txtUom+'" readonly /></td>'+
                            '<td align="center"><div class="form-check">'+
                                '<input name="isCheck[]" class="form-check-input" type="checkbox" value="1" '+(val.intIsCheck == 1?'checked':'')+(role == 4?' disabled':'')+'/>'+
                            '</div></td>'+
                        '</tr>';
                })
