...
@push('scripts')
    ...
    <script>
        ...
        var daTable = $('#daTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('ftq.admin.access.index') }}",
            columns: [
        ...
        function edit(id){
            ...
            let userUrl = "{{ route('ftq.admin.access.users', ':id') }}";
            ...
            $.get(userUrl, function(response){
                ...
                let editUrl = "{!! route('ftq.admin.access.show', ':id') !!}";
                editUrl = editUrl.replace(':id', id);
                url = "{{ route('ftq.admin.access.update', ':id') }}";
                url = url.replace(':id', id);
                method = "POST";
                $.get(editUrl, function(response){
                    $('#modal-level').modal('show');
                    $('input[name="intLevel_ID"]').val(response.data.intLevel_ID);
        ...
        <div class="modal-body">
          <form action="" method="post" id="form-level" data-parsley-validate="true">
            <div class="mb-3">
                ...
                <input class="form-control" type="text" name="txtLevelName" id="LevelName" placeholder="Level Name" oninput="this.value = this.value.toUpperCase()" disabled/>
            </div>
            <div class="mb-3">
