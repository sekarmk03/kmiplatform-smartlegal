...
@section('title', 'Manage Custom Parameter')
@push('css')
...
@push('scripts')
    ...
    <script>
        ...
        var daTable = $('#daTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('ftq.admin.custom-parameter.index') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'dtmCreatedAt', name: 'dtmCreatedAt'},
                {data: 'txtCreatedBy', name: 'txtCreatedBy'},
                {data: 'txtCustomValue', name: 'txtCustomValue'},
        ...
        function create(){
            $('.modal-header h4').html('Create Custom Parameter');
            $('#modal-level').modal('show');
            url = "{{ route('ftq.admin.custom-parameter.store') }}";
            method = "POST";
        }
        function edit(id){
            $('.modal-header h4').html('Edit Custom Parameter');
            $('.modal-body form').append('<input type="hidden" name="_method" value="PUT">');
            let editUrl = "{!! route('ftq.admin.custom-parameter.edit', ':id') !!}";
            editUrl = editUrl.replace(':id', id);
            url = "{{ route('ftq.admin.custom-parameter.update', ':id') }}";
            url = url.replace(':id', id);
            method = "POST";
            $.get(editUrl, function(response){
                $('#modal-level').modal('show');
                $('input[name="txtCustomValue"]').val(response.data.txtCustomValue);
            }).fail(function(response){
                notification(response.responseJSON.status, response.responseJSON.message,'bg-danger');
            });
        }
        function destroy(id){
            let deleteUrl = "{{ route('ftq.admin.custom-parameter.destroy', ':id') }}";
...
@section('content')
	...
	<ol class="breadcrumb float-xl-end">
		...
		<li class="breadcrumb-item active">Manage Custom Parameter</li>
	</ol>
	<!-- END breadcrumb -->
	<!-- BEGIN page-header -->
	<h1 class="page-header">Manage Custom Parameter</h1>
	<!-- END page-header -->
    <div class="row">
        <div class="col">
            <div class="panel panel-inverse">
                <div class="panel-heading">
                    <h4 class="panel-title">Custom Parameter Table</h4>
                ...
                <div class="panel-body">
                    <div class="row my-3">
                        <div class="col-md-2 ms-auto">
                            <button class="btn btn-sm btn-success float-end" onclick="create()"><i class="fa-solid fa-plus"></i> New Custom Parameter</button>
                        </div>
                    </div>
                    <div class="row">
                        <!-- html -->
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="" method="post" id="form-level" data-parsley-validate="true">
            <div class="mb-3">
                <label class="form-label" id="ParameterValue">Parameter Value</label>
                <input class="form-control" type="text" name="txtCustomValue" id="ParameterValue" placeholder="ParameterValue"/>
            </div>
        </div>
