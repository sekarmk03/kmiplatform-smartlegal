...
@push('scripts')
    ...
    <script>
        ...
        var daTable = $('#daTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('roonline.line.index') }}",
            columns: [
        ...
        function create(){
            ...
            url = "{{ route('roonline.line.store') }}";
            method = "POST";
        }
        function edit(id){
            $('.modal-header h4').html('Edit Line');
            $('.modal-body form').append('<input type="hidden" name="_method" value="PUT">');
            let editUrl = "{!! route('roonline.line.edit', ':id') !!}";
            editUrl = editUrl.replace(':id', id);
            url = "{{ route('roonline.line.update', ':id') }}";
        ...
        function destroy(id){
            let deleteUrl = "{{ route('roonline.line.destroy', ':id') }}";
