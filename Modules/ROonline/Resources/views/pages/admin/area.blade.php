...
@push('scripts')
    ...
    <script>
        ...
        var daTable = $('#daTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('roonline.area.index') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex'},
                {data: 'dtmCreatedAt', name: 'dtmCreatedAt'},
                {data: 'txtAreaName', name: 'txtAreaName'},
        ...
        function create(){
            $('.modal-header h4').html('Create Area');
            $('#modal-level').modal('show');
            url = "{{ route('roonline.area.store') }}";
            method = "POST";
        }
        function edit(id){
            $('.modal-header h4').html('Edit Area');
            $('.modal-body form').append('<input type="hidden" name="_method" value="PUT">');
            let editUrl = "{!! route('roonline.area.edit', ':id') !!}";
            editUrl = editUrl.replace(':id', id);
            url = "{{ route('roonline.area.update', ':id') }}";
            url = url.replace(':id', id);
            method = "POST";
            $.get(editUrl, function(response){
                $('#modal-level').modal('show');
                $('input#AreaName').val(response.data.txtAreaName);
            }).fail(function(response){
                notification(response.responseJSON.status, response.responseJSON.message,'bg-danger');
            });
        }
        function destroy(id){
            let deleteUrl = "{{ route('roonline.area.destroy', ':id') }}";
...
@section('content')
	...
	<!-- END breadcrumb -->
	<!-- BEGIN page-header -->
	<h1 class="page-header">Manage Areas</h1>
	<!-- END page-header -->
    <div class="row">
        <div class="col">
            <div class="panel panel-inverse">
                <div class="panel-heading">
                    <h4 class="panel-title">Areas Table</h4>
                ...
                <div class="panel-body">
                    <div class="row my-3">
                        <div class="col-md-2 ms-auto">
                            <button class="btn btn-sm btn-success float-end" onclick="create()"><i class="fa-solid fa-plus"></i> New Areas</button>
                        </div>
                    </div>
                    <div class="row">
                        <!-- html -->
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="" method="post" id="form-level" data-parsley-validate="true">
            <div class="mb-3">
                <label class="form-label" id="AreaName">Area Name</label>
                <input class="form-control" type="text" name="txtAreaName" id="AreaName" placeholder="Area Name" oninput="this.value = this.value.toUpperCase()"/>
            </div>
        </div>
