...
@push('scripts')
    ...
    <script>
        ...
        var daTable = $('#daTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('rois.manage.inspection') }}",
            columns: [
        ...
        function edit(id){
            ...
            let editUrl = "{!! route('rois.manage.inspection.edit', ':id') !!}";
            editUrl = editUrl.replace(':id', id);
            url = "{{ route('rois.manage.inspection.update', ':id') }}";
        ...
        function BatchOrderOption(){
            ...
            let opt = '';
            $.get("{{ route('rois.inspection.okp') }}", function(response){
        ...
        $(document).ready(function(){
            $('select#BatchOrder').on('sp-change', function(){
                $.get("{{ route('rois.inspection.okp') }}", {batch: $(this).val()}, function(response){
                    let data = response.data;
                    $('input#Product').val(data.DESCRIPTION);
                    $('input#ProductionCode').val(data.LOT_NUMBER);
                    $('input#ExpireDate').val(data.EXPIRATION_DATE);
                }).then(() => {
                    $.post("{{ route('rois.inspection.lot') }}", {batch: $(this).val()}, function(response){
                        let data = response.data;
                        lotOption(data);
                    })
                })
        ...
        <div class="modal-body">
          <form action="" method="post" id="form-level" data-parsley-validate="true">
            ...
            <div class="mb-3">
                ...
                <input class="form-control" type="text" name="txtProductName" id="Product" placeholder="Product" readonly/>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="dtmExpireDate" id="ExpireDate" class="form-control" data-date-format="yyyy-mm-dd" readonly>
            </div>
            <div class="mb-3">
