        ...
        <div class="col-12 ui-sortable">
            <div class="panel panel-inverse">
                <div class="panel-heading">
                    <h4 class="panel-title">Issuer Table</h4>
                    <div class="me-3">
    ...
    <div class="modal fade" id="modal-level">
        ...
        <div class="modal-content">
            ...
            <form action="" method="post">
                    ...
                    <div class="mb-3">
                        <label class="form-label" for="IssuerDesc">Description</label>
                        <textarea name="txtDesc" id="IssuerDesc" cols="30" rows="5" class="form-control" placeholder="Enter description.."></textarea>
                    </div>
            </div>
...
@push('scripts')
    ...
    <script>
        ...
        let daTable = $('#daTable').DataTable({
            ...
            autoWidth: true,
            responsive: true,
            ajax: "{{ route('smartlegal.master.issuer.index') }}",
            columns: [
                {data: 'DT_RowIndex', name: 'DT_RowIndex', className: 'text-center'},
                {data: 'dtmCreatedAt', name: 'dtmCreatedAt', className: 'text-center'},
                {data: 'txtIssuerName', name: 'txtIssuerName', className: 'text-center'},
                {data: 'txtCode', name: 'txtCode', className: 'text-center'},
                {
                    data: 'txtDesc',
                    name: 'txtDesc',
        ...
        const edit = ( id ) => {
            ...
            $.get(editUrl, (response) => {
                $('#modal-level').modal('show');
                $('input#IssuerName').val(response.data.txtIssuerName);
                $('input#IssuerCode').val(response.data.txtCode);
                $('textarea#IssuerDesc').val(response.data.txtDesc);
            });
        }
        ...
        const destroy = (id) => {
            let deleteUrl = "{{ route('smartlegal.master.issuer.destroy', ':id') }}";
        ...
        $(document).ready(() => {
            ...
            $('#modal-level').on('hide.bs.modal', () => {
                $('input#IssuerName').val('');
                $('input#IssuerCode').val('');
                $('textarea#IssuerDesc').val('');
