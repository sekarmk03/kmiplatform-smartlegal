...
@push('scripts')
    ...
    <script>
        ...
        var daTable = $('#daTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('manage.subdepartment.index') }}",
            columns: [
        ...
        function destroy(id){
            let deleteUrl = "{{ route('manage.subdepartment.destroy', ':id') }}";
